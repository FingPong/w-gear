<?php
    ...
    if (isset($_GET['rep'])) {
        $sql_read = "UPDATE tb_comment SET status=1 WHERE id={$_GET['rep']}";
        mysqli_query($mysqli, $sql_read);
?>
    <script>
...
<div class="grid detail-form__product Keyboards" id="detail-form"> 
    <!-- <form action=""> -->
        ...
        <div class="detail-form__product-body">
            ...
            <div class="detail-form_product-comment">
                <div class="detail-form__product-configuration-heading detail-form__product-blog-heading">
                    <h4>BÌNH LUẬN</h4>
                </div>
                <div class="body" id="comment-body">
                    <div class="comment">
                        <!-- <form action="" method="POST">  -->
                            <div class="comment-info">
                                <div class="comment-info-box">
                                    <input type="text" class="input name" name="name" id='name' placeholder="Nhập họ tên" required>
                                    <?php echo $msgName; ?>
                                </div>
                                <div class="comment-info-box">
                                    <input type="text" class="input phone" name="phone" id='phone' placeholder="Nhập số điện thoại" required>
                                    <?php echo $msgPhone; ?>
                                </div>
                            </div>
                    ...
                    <div class="others-comment" id="others-comment">
                        <?php
                            $sql_cmt = "SELECT * FROM tb_comment WHERE id_product={$row['id']} AND reply=0 ORDER BY date DESC";
...
<script>
    function repComment(id) {
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                document.getElementById("comment-body").innerHTML = this.responseText;
            } 
        };   
        // console.log(path);
        name = '<?php if (isset($_SESSION['USERNAME'])) echo $row_admin['name']; ?>'; 
        content = document.getElementById(id).value;
        var path = `pages/main/comment.php?id=<?php echo $row['id']; ?>&reptoid=${id}&name=${name}&content=${content}`;
        // alert(path);
        xhttp.open("GET", path, true);
        xhttp.send(); 
    }
    ...
    function comment() {
        ...
        if (name != '' && phone != '' && content != '') {
            var path = `pages/main/comment.php?id=<?php echo $row['id']; ?>&name=${name}&phone=${phone}&content=${content}`;
            // alert(path);
            xhttp.open("GET", path, true);
            xhttp.send();
        }
