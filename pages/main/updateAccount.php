<?php 
    ...
    if (isset($_GET['accept-order'])) {
        ...
        // $sql_comment = "SELECT * FROM tb_comment WHERE id_cart = {$_GET['accept-order']}";
        // $query_comment = mysqli_query($mysqli, $sql_comment);
        // while ($row_comment = mysqli_fetch_array($query_comment)) {
        //     $sql_update_comment = "UPDATE tb_cart_detail SET comment = 1 WHERE id_cart={$_GET['accept-order']}";
        //     mysqli_query($mysqli, $sql_update_comment);
        // }
        header("Location: ../../index.php?taikhoan&donhang");
    }
