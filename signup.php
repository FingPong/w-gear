<?php
    ...
    if (isset($_POST["submit"])) {
        ...
        if (strlen($password) < 8) {
            $msgPassword = "<span class='error-message'>Mật khẩu phải trên 8 kí tự!</span>";
            $isValid = 0;
        }
        else if (!preg_match("/[0-9]/",$password)) {
            $msgPassword = "<span class='error-message'>Mật khẩu phải có ít nhất 1 chữ số 0 - 9!</span>";
            $isValid = 0;
        }
        else if (!preg_match("/[a-z]/",$password)) {
            $msgPassword = "<span class='error-message'>Mật khẩu phải có ít nhất 1 chữ cái thường a - z!</span>";
            $isValid = 0;
        }
        else if (!preg_match("/[A-Z]/",$password)) {
            $msgPassword = "<span class='error-message'>Mật khẩu phải có ít nhất 1 chữ cái hoa A - Z!</span>";
            $isValid = 0;
        }
        ...
        if ($isValid) {
            ...
            if ($query) {
                ...
                try {
                    ...
                    $mail->SMTPAuth   = true;                                  
                    $mail->Username   = 'wgear.official.contact@gmail.com';           
                    $mail->Password   = 'glzpjstukmsvpvpm';                           
                    $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;       
                    $mail->Port       = 465;                         

                    //Recipients
                    $mail->setFrom('wgear.official.contact@gmail.com');
